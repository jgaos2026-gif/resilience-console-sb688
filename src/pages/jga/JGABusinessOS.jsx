...
import {
  ...
  Brain, Sparkles, Landmark, ReceiptText, Scale, TrendingUp,
  Heart, PhoneCall, Palette
} from "lucide-react";
...
const TABS = [
  ...
  { id: "ai_finance", label: "AI Finance Hub", icon: Landmark },
  { id: "branding",   label: "Brand Engine",   icon: Palette },
...
export default function JGABusinessOS() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-xl overflow-x-auto" style={{ background: "#0d0f1a", border: `1px solid ${GOLD}20` }}>
        {TABS.map(t => {
          const Icon = t.icon;
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-[11px] font-bold whitespace-nowrap transition-all"
              style={tab === t.id
                ? { background: `linear-gradient(135deg, ${GOLD}, #8a6018)`, color: "#0a0c10" }
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border p-5 space-y-4" style={{ background: "hsl(220,18%,7%)" }}>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold font-cinzel" style={{ color: GOLD }}>AI-Powered Business Capabilities</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                { icon: Landmark,    color: "#60a5fa", label: "Banking & Record Keeping",     desc: "Cash flow analysis, daily/weekly/monthly tracking, account health" },
                { icon: ReceiptText, color: "#4ade80", label: "Taxes & Write-Offs",           desc: "Year-end filing, quarterly estimates, Illinois deductions, write-off maximization" },
                { icon: Scale,       color: "#a78bfa", label: "Compliance & Legal",           desc: "LLC requirements, contracts, IP ownership, contractor classification" },
                { icon: TrendingUp,  color: GOLD,      label: "Investment & Angel Repayment", desc: "Pitch strategy, repayment structures, investor metrics, valuation" },
                { icon: Heart,       color: "#ef4444", label: "Charity & Donations",          desc: "Tax-advantaged giving, in-kind services, brand-building philanthropy" },
                { icon: PhoneCall,   color: "#f97316", label: "Sales & Follow-Ups",           desc: "Follow-up sequences, cold outreach scripts, upsells, referral programs" },
                { icon: Palette,     color: "#ec4899", label: "Psychological Brand Engine",   desc: "Deep questionnaire reveals your perfect brand identity, archetype, palette, tagline" },
                { icon: Sparkles,    color: "#06b6d4", label: "AI Strategy Calls",            desc: "AI-simulated sales calls, objection handling, negotiation playbooks" },
                { icon: Building2,   color: "#84cc16", label: "Business OS Dashboard",       desc: "All systems unified — orders, clients, contractors, payments in one view" },
              ].map((cap, i) => {
                const Icon = cap.icon;
                return (
                  <div key={i} className="flex items-start gap-3 p-3 rounded-xl border border-border/50 hover:border-border transition-all" style={{ background: "rgba(0,0,0,0.3)" }}>
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${cap.color}15`, border: `1px solid ${cap.color}25` }}>
                      <Icon className="w-4 h-4" style={{ color: cap.color }} />
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-foreground">{cap.label}</p>
                      <p className="text-[9px] text-muted-foreground mt-0.5 leading-relaxed">{cap.desc}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="rounded-xl border border-border p-5 space-y-3" style={{ background: "hsl(220,18%,7%)" }}>
            <h2 className="text-sm font-bold font-cinzel" style={{ color: GOLD }}>Business Rules & Policies</h2>
      ...
      {tab === "ai_finance" && (
        <div className="space-y-4">
          <div className="rounded-xl border p-4 flex items-start gap-3" style={{ background: `${GOLD}08`, borderColor: `${GOLD}25` }}>
            <Landmark className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: GOLD }} />
            <div>
              <p className="text-sm font-bold font-cinzel" style={{ color: GOLD }}>AI Finance Hub</p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Click any module to generate a live AI analysis for JGA Enterprises based on your actual business data.
                Covers banking, taxes, compliance, investment, charity, and sales.
              </p>
            </div>
          </div>
          <AIFinancePanel payments={payments} clients={clients} orders={orders} contractors={contractors} />
        </div>
      )}
      ...
      {tab === "branding" && (
        <div className="space-y-4">
          <div className="rounded-xl border p-4 flex items-start gap-3" style={{ background: "rgba(236,72,153,0.05)", borderColor: "rgba(236,72,153,0.25)" }}>
            <Palette className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: "#ec4899" }} />
            <div>
              <p className="text-sm font-bold font-cinzel" style={{ color: "#ec4899" }}>Psychological Brand Engine</p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Answer 9 deep psychological questions and our AI will engineer your perfect brand identity —
                archetype, color palette, taglines, logo concept, positioning, and your brand's soul.
              </p>
            </div>
          </div>
          <div className="rounded-xl border border-border p-5" style={{ background: "hsl(220,18%,7%)" }}>
      ...
      {tab === "orders" && (
        <div className="rounded-xl border border-border overflow-hidden" style={{ background: "hsl(220,18%,7%)" }}>
          <div className="p-4 border-b border-border">
            <h2 className="text-sm font-bold font-cinzel" style={{ color: GOLD }}>All Orders</h2>
          </div>
                ...
                {orders.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="border-b border-border/30 hover:bg-secondary/20">
                      ...
                      <td className="p-3">
                        <Badge className="text-[8px] border" style={order.track === "system_a"
                          ? { background: "rgba(96,165,250,0.1)", color: "#60a5fa", borderColor: "rgba(96,165,250,0.3)" }
                          : { background: "rgba(167,139,250,0.1)", color: "#a78bfa", borderColor: "rgba(167,139,250,0.3)" }}>
